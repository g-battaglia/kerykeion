...
def test_the_exemption_list_does_not_outlive_its_reason():
    """An exemption for a file that no longer exists, or that a test compares, is a lie.

    A file some golden test hands to the comparison is regenerated by running that
    test with KERYKEION_REGEN_BASELINES set, so "cannot regenerate here" is refuted
    for it whatever the reason says.
    """
