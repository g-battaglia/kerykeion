...
def _compare_chart_svg(file_name: str, chart_svg: str) -> None:
    ...
    Was a second copy of the comparison, loosened again at the call site to
    rel_tol=0.5; now the one comparison, like everywhere else. A missing baseline
    still skips here and only here: this matrix is generated by
    regenerate_test_charts_extended.py and a combination may legitimately have no
    file yet, which is not the same as a golden test whose baseline vanished.
    """
    baseline_path = SVG_DIR / file_name
    if not baseline_path.exists():
        pytest.skip(f"Baseline file not found: {file_name}. Run `uv run poe regenerate:svg`.")
    compare_svg_file(baseline_path, chart_svg)
