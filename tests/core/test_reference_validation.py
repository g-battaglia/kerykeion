...
_REFERENCE_SUBJECTS = [
    ...
    ("BuenosAires1951i016", 1951, 9, 9, 16, 32, -34.6, -58.38, "America/Argentina/Buenos_Aires", "2072-06-15", (1952, 1, 8, 13, 56)),
    ("BuenosAires1961i042", 1961, 7, 15, 18, 54, -34.6, -58.38, "America/Argentina/Buenos_Aires", "2038-06-15", (1961, 9, 30, 19, 53)),
    ("BuenosAires1971i068", 1971, 5, 21, 20, 16, -34.6, -58.38, "America/Argentina/Buenos_Aires", "2060-06-15", (1971, 8, 19, 0, 56)),
    ("BuenosAires1981i094", 1981, 3, 27, 22, 38, -34.6, -58.38, "America/Argentina/Buenos_Aires", "2026-06-15", (1981, 5, 12, 6, 49)),
    ("BuenosAires1991i120", 1991, 1, 5, 0, 0, -34.6, -58.38, "America/Argentina/Buenos_Aires", "2048-06-15", (1991, 3, 3, 12, 39)),
    ...
    ("LosAngeles1926i037", 1926, 6, 16, 1, 29, 34.05, -118.24, "America/Los_Angeles", "2079-06-15", (1926, 11, 16, 9, 27)),
    ("LosAngeles1936i063", 1936, 4, 22, 3, 51, 34.05, -118.24, "America/Los_Angeles", "2045-06-15", (1936, 8, 9, 15, 24)),
    ("LosAngeles1946i089", 1946, 2, 28, 5, 13, 34.05, -118.24, "America/Los_Angeles", "2067-06-15", (1946, 6, 29, 20, 15)),
    ("LosAngeles1956i115", 1956, 12, 6, 7, 35, 34.05, -118.24, "America/Los_Angeles", "2033-06-15", (1957, 2, 21, 4, 7)),
    ("LosAngeles2002i011", 2002, 8, 10, 23, 7, 34.05, -118.24, "America/Los_Angeles", "2057-06-15", (2002, 10, 5, 2, 23)),
    ...
    ("MexicoCity1937i014", 1937, 11, 15, 14, 58, 19.43, -99.13, "America/Mexico_City", "2066-06-15", (1938, 3, 24, 10, 54)),
    ("MexicoCity1947i040", 1947, 9, 21, 16, 20, 19.43, -99.13, "America/Mexico_City", "2032-06-15", (1947, 12, 15, 15, 55)),
    ("MexicoCity1957i066", 1957, 7, 27, 18, 42, 19.43, -99.13, "America/Mexico_City", "2054-06-15", (1957, 11, 1, 21, 54)),
    ("MexicoCity1967i092", 1967, 5, 5, 20, 4, 19.43, -99.13, "America/Mexico_City", "2076-06-15", (1967, 8, 23, 4, 47)),
    ("MexicoCity1977i118", 1977, 3, 11, 22, 26, 19.43, -99.13, "America/Mexico_City", "2042-06-15", (1977, 5, 16, 10, 40)),
    ...
    ("SaoPaulo1984i119", 1984, 8, 22, 11, 43, -23.55, -46.63, "America/Sao_Paulo", "2045-06-15", (1984, 10, 22, 10, 13)),
...
def _natal(name, y, mo, d, h, mn, lat, lng, tz):
    return AstrologicalSubjectFactory.from_birth_data(
        name, y, mo, d, h, mn, lng=lng, lat=lat, tz_str=tz, online=False,
    )
...
def test_reference_sp_ephemeris_date(row):
    ...
    assert diff < _REFERENCE_TOLERANCE_S, (
        f"{row[0]}: SP ephemeris off by {diff:.0f}s — ours={our} reference={ref}"
    )
...
def test_reference_orb_config_monotonic(row):
    ...
    for orb in (1.0, 2.0, 3.0, 5.0):
        sp = SecondaryProgressionFactory.compute_full(
            natal, target_iso_utc_datetime=_target_iso(row), aspect_orb=orb,
        )
...
def test_reference_predictive_aspects_ptolemaic(row):
    ...
    sp = SecondaryProgressionFactory.compute_full(
        natal, target_iso_utc_datetime=_target_iso(row), aspect_orb=3.0,
    )
