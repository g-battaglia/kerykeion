...
from tests.tolerance_config import (
    POSITION_TOL,
    SPEED_TOL,
    DECLINATION_TOL,
    TRUE_NODE_TOL,
...
class TestAstrologicalSubject:
    ...
    def test_internal_data(self):
        assert self.subject.lat == approx(self.expected_output["lat"], abs=POSITION_TOL)
        assert self.subject.lng == approx(self.expected_output["lng"], abs=POSITION_TOL)
    ...
    def test_ascendant(self):
        ...
        assert self.subject.ascendant.position == approx(
            self.expected_output["ascendant"]["position"], abs=POSITION_TOL
        )
        assert self.subject.ascendant.abs_pos == approx(self.expected_output["ascendant"]["abs_pos"], abs=POSITION_TOL)
    ...
    def test_descendant(self):
        ...
        assert self.subject.descendant.position == approx(
            self.expected_output["descendant"]["position"], abs=POSITION_TOL
        )
        assert self.subject.descendant.abs_pos == approx(
            self.expected_output["descendant"]["abs_pos"], abs=POSITION_TOL
        )
    ...
    def test_medium_coeli(self):
        ...
        assert self.subject.medium_coeli.position == approx(
            self.expected_output["medium_coeli"]["position"], abs=POSITION_TOL
        )
        assert self.subject.medium_coeli.abs_pos == approx(
            self.expected_output["medium_coeli"]["abs_pos"], abs=POSITION_TOL
        )
    ...
    def test_imum_coeli(self):
        ...
        assert self.subject.imum_coeli.position == approx(
            self.expected_output["imum_coeli"]["position"], abs=POSITION_TOL
        )
        assert self.subject.imum_coeli.abs_pos == approx(
            self.expected_output["imum_coeli"]["abs_pos"], abs=POSITION_TOL
        )
    ...
    def test_sun(self):
        ...
        assert self.subject.sun.position == approx(self.expected_output["sun"]["position"], abs=POSITION_TOL)
        assert self.subject.sun.abs_pos == approx(self.expected_output["sun"]["abs_pos"], abs=POSITION_TOL)
        ...
        assert self.subject.sun.speed == approx(self.expected_output["sun"]["speed"], abs=SPEED_TOL)
        assert self.subject.sun.declination == approx(self.expected_output["sun"]["declination"], abs=DECLINATION_TOL)
    ...
    def test_moon(self):
        ...
        assert self.subject.moon.position == approx(self.expected_output["moon"]["position"], abs=POSITION_TOL)
        assert self.subject.moon.abs_pos == approx(self.expected_output["moon"]["abs_pos"], abs=POSITION_TOL)
        ...
        assert self.subject.moon.speed == approx(self.expected_output["moon"]["speed"], abs=SPEED_TOL)
        assert self.subject.moon.declination == approx(self.expected_output["moon"]["declination"], abs=POSITION_TOL)
    ...
    def test_mercury(self):
        ...
        assert self.subject.mercury.position == approx(self.expected_output["mercury"]["position"], abs=POSITION_TOL)
        assert self.subject.mercury.abs_pos == approx(self.expected_output["mercury"]["abs_pos"], abs=POSITION_TOL)
        ...
        assert self.subject.mercury.speed == approx(self.expected_output["mercury"]["speed"], abs=SPEED_TOL)
        assert self.subject.mercury.declination == approx(
            self.expected_output["mercury"]["declination"], abs=POSITION_TOL
        )
    ...
    def test_venus(self):
        ...
        assert self.subject.venus.position == approx(self.expected_output["venus"]["position"], abs=POSITION_TOL)
        assert self.subject.venus.abs_pos == approx(self.expected_output["venus"]["abs_pos"], abs=POSITION_TOL)
        ...
        assert self.subject.venus.speed == approx(self.expected_output["venus"]["speed"], abs=SPEED_TOL)
        assert self.subject.venus.declination == approx(self.expected_output["venus"]["declination"], abs=POSITION_TOL)
    ...
    def test_mars(self):
        ...
        assert self.subject.mars.position == approx(self.expected_output["mars"]["position"], abs=POSITION_TOL)
        assert self.subject.mars.abs_pos == approx(self.expected_output["mars"]["abs_pos"], abs=POSITION_TOL)
        ...
        assert self.subject.mars.speed == approx(self.expected_output["mars"]["speed"], abs=SPEED_TOL)
        assert self.subject.mars.declination == approx(self.expected_output["mars"]["declination"], abs=POSITION_TOL)
    ...
    def test_jupiter(self):
        ...
        assert self.subject.jupiter.position == approx(self.expected_output["jupiter"]["position"], abs=POSITION_TOL)
        assert self.subject.jupiter.abs_pos == approx(self.expected_output["jupiter"]["abs_pos"], abs=POSITION_TOL)
        ...
        assert self.subject.jupiter.speed == approx(self.expected_output["jupiter"]["speed"], abs=SPEED_TOL)
        assert self.subject.jupiter.declination == approx(
            self.expected_output["jupiter"]["declination"], abs=POSITION_TOL
        )
    ...
    def test_saturn(self):
        ...
        assert self.subject.saturn.position == approx(self.expected_output["saturn"]["position"], abs=POSITION_TOL)
        assert self.subject.saturn.abs_pos == approx(self.expected_output["saturn"]["abs_pos"], abs=POSITION_TOL)
        ...
        assert self.subject.saturn.speed == approx(self.expected_output["saturn"]["speed"], abs=SPEED_TOL)
        assert self.subject.saturn.declination == approx(
            self.expected_output["saturn"]["declination"], abs=POSITION_TOL
        )
    ...
    def test_uranus(self):
        ...
        assert self.subject.uranus.position == approx(self.expected_output["uranus"]["position"], abs=POSITION_TOL)
        assert self.subject.uranus.abs_pos == approx(self.expected_output["uranus"]["abs_pos"], abs=POSITION_TOL)
        ...
        assert self.subject.uranus.speed == approx(self.expected_output["uranus"]["speed"], abs=SPEED_TOL)
        assert self.subject.uranus.declination == approx(
            self.expected_output["uranus"]["declination"], abs=POSITION_TOL
        )
    ...
    def test_neptune(self):
        ...
        assert self.subject.neptune.position == approx(self.expected_output["neptune"]["position"], abs=POSITION_TOL)
        assert self.subject.neptune.abs_pos == approx(self.expected_output["neptune"]["abs_pos"], abs=POSITION_TOL)
        ...
        assert self.subject.neptune.speed == approx(self.expected_output["neptune"]["speed"], abs=SPEED_TOL)
        assert self.subject.neptune.declination == approx(
            self.expected_output["neptune"]["declination"], abs=POSITION_TOL
        )
    ...
    def test_pluto(self):
        ...
        assert self.subject.pluto.position == approx(self.expected_output["pluto"]["position"], abs=POSITION_TOL)
        assert self.subject.pluto.abs_pos == approx(self.expected_output["pluto"]["abs_pos"], abs=POSITION_TOL)
        ...
        assert self.subject.pluto.speed == approx(self.expected_output["pluto"]["speed"], abs=SPEED_TOL)
        assert self.subject.pluto.declination == approx(self.expected_output["pluto"]["declination"], abs=POSITION_TOL)
    ...
    def test_first_house(self):
        ...
        assert self.subject.first_house.position == approx(
            self.expected_output["first_house"]["position"], abs=POSITION_TOL
        )
        assert self.subject.first_house.abs_pos == approx(
            self.expected_output["first_house"]["abs_pos"], abs=POSITION_TOL
        )
    ...
    def test_second_house(self):
        ...
        assert self.subject.second_house.position == approx(
            self.expected_output["second_house"]["position"], abs=POSITION_TOL
        )
        assert self.subject.second_house.abs_pos == approx(
            self.expected_output["second_house"]["abs_pos"], abs=POSITION_TOL
        )
    ...
    def test_third_house(self):
        ...
        assert self.subject.third_house.position == approx(
            self.expected_output["third_house"]["position"], abs=POSITION_TOL
        )
        assert self.subject.third_house.abs_pos == approx(
            self.expected_output["third_house"]["abs_pos"], abs=POSITION_TOL
        )
    ...
    def test_fourth_house(self):
        ...
        assert self.subject.fourth_house.position == approx(
            self.expected_output["fourth_house"]["position"], abs=POSITION_TOL
        )
        assert self.subject.fourth_house.abs_pos == approx(
            self.expected_output["fourth_house"]["abs_pos"], abs=POSITION_TOL
        )
    ...
    def test_fifth_house(self):
        ...
        assert self.subject.fifth_house.position == approx(
            self.expected_output["fifth_house"]["position"], abs=POSITION_TOL
        )
        assert self.subject.fifth_house.abs_pos == approx(
            self.expected_output["fifth_house"]["abs_pos"], abs=POSITION_TOL
        )
    ...
    def test_sixth_house(self):
        ...
        assert self.subject.sixth_house.position == approx(
            self.expected_output["sixth_house"]["position"], abs=POSITION_TOL
        )
        assert self.subject.sixth_house.abs_pos == approx(
            self.expected_output["sixth_house"]["abs_pos"], abs=POSITION_TOL
        )
    ...
    def test_seventh_house(self):
        ...
        assert self.subject.seventh_house.abs_pos == approx(
            self.expected_output["seventh_house"]["abs_pos"], abs=POSITION_TOL
        )
    ...
    def test_eighth_house(self):
        ...
        assert self.subject.eighth_house.position == approx(
            self.expected_output["eighth_house"]["position"], abs=POSITION_TOL
        )
        assert self.subject.eighth_house.abs_pos == approx(
            self.expected_output["eighth_house"]["abs_pos"], abs=POSITION_TOL
        )
    ...
    def test_ninth_house(self):
        ...
        assert self.subject.ninth_house.position == approx(
            self.expected_output["ninth_house"]["position"], abs=POSITION_TOL
        )
        assert self.subject.ninth_house.abs_pos == approx(
            self.expected_output["ninth_house"]["abs_pos"], abs=POSITION_TOL
        )
    ...
    def test_tenth_house(self):
        ...
        assert self.subject.tenth_house.position == approx(
            self.expected_output["tenth_house"]["position"], abs=POSITION_TOL
        )
        assert self.subject.tenth_house.abs_pos == approx(
            self.expected_output["tenth_house"]["abs_pos"], abs=POSITION_TOL
        )
    ...
    def test_twelfth_house(self):
        ...
        assert self.subject.twelfth_house.abs_pos == approx(
            self.expected_output["twelfth_house"]["abs_pos"], abs=POSITION_TOL
        )
...
class TestAstrologicalSubjectFactoryMethods:
    ...
    def test_from_iso_utc_time_offline(self):
        ...
        assert subject.lat == approx(40.7128, abs=POSITION_TOL)
        assert subject.lng == approx(-74.006, abs=POSITION_TOL)
...
class TestEdgeCases:
    ...
    def test_southern_hemisphere_coordinates(self):
        ...
        assert subject.lat == approx(-33.8688, abs=POSITION_TOL)
        assert subject.lng == approx(151.2093, abs=POSITION_TOL)
    ...
    def test_western_hemisphere_coordinates(self):
        ...
        assert subject.lat == approx(34.0522, abs=POSITION_TOL)
        assert subject.lng == approx(-118.2437, abs=POSITION_TOL)
    ...
    def test_near_date_line(self):
        ...
        assert subject.lng == approx(179.0, abs=POSITION_TOL)
...
class TestVertexCalculation:
    ...
    def test_both_vertex_and_anti_vertex(self):
        ...
        assert subject.anti_vertex.abs_pos == approx(expected_anti, abs=POSITION_TOL)
...
class TestMockErrorConditions:
    ...
    @pytest.mark.skip(reason="Requires swisseph module - not available with LibEphemeris")
