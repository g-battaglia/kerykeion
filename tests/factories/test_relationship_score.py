...
class TestRelationshipScore:
    def setup_class(self):
        self.john_lennon = AstrologicalSubjectFactory.from_birth_data(
            "John", 1940, 10, 9, 18, 30, "Liverpool", "UK", suppress_geonames_warning=True
        )
        self.yoko_ono = AstrologicalSubjectFactory.from_birth_data(
            "Yoko", 1933, 2, 18, 20, 30, "Tokyo", "JP", suppress_geonames_warning=True
        )

        self.freud = AstrologicalSubjectFactory.from_birth_data(
            "Freud", 1856, 5, 6, 18, 30, "Freiberg", "DE", suppress_geonames_warning=True
        )
        self.jung = AstrologicalSubjectFactory.from_birth_data(
            "Jung", 1875, 7, 26, 18, 30, "Kesswil", "CH", suppress_geonames_warning=True
        )

        self.richart_burton = AstrologicalSubjectFactory.from_birth_data(
            "Richard Burton", 1925, 11, 10, 15, 00, "Pontrhydyfen", "UK", suppress_geonames_warning=True
        )
        self.liz_taylor = AstrologicalSubjectFactory.from_birth_data(
            "Elizabeth Taylor", 1932, 2, 27, 2, 30, "London", "UK", suppress_geonames_warning=True
        )

        self.dario_fo = AstrologicalSubjectFactory.from_birth_data(
            "Dario Fo", 1926, 3, 24, 12, 25, "Sangiano", "IT", suppress_geonames_warning=True
        )
        self.franca_rame = AstrologicalSubjectFactory.from_birth_data(
