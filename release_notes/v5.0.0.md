...
-   `PlanetaryReturnFactory` computes Solar and Lunar returns via Swiss Ephemeris (main method: `next_return_from_date`, plus `next_return_from_iso_formatted_time`).
...
-   The `examples/` folder now ships runnable scripts (see _Quickstart examples_ below) aligned with the docs.
...
-   Upgrade quickstart: see the _Migration Guide_ inside `README.md`
