...
- `AspectsFactory` unifies single- and dual-chart aspects with axis filters (`DEFAULT_AXIS_ORBIT = 1`) and keeps legacy aliases (`NatalAspectsModel`, `SynastryAspectsModel`) for compatibility.
...
- `DEFAULT_ACTIVE_POINTS` and `DEFAULT_ACTIVE_ASPECTS` are more restrictive; aspects involving chart axes use the dedicated 1° orb.
...
#### 4. Update Custom Settings/Configurations

If you have custom `kr.config.json` files or language settings, update all lunar node references:
...
- `kerykeion/settings` introduces legacy presets (`settings/legacy/*`) and caches configuration merges via `functools.lru_cache`.
