...
- **Nakshatras on a non-sidereal chart are corrected.** v5 fed tropical longitudes straight to the sidereal 27-fold division, so every `nakshatra`/`nakshatra_pada`/`nakshatra_lord` came out about two nakshatras off. v6 rotates them by `nakshatra_ayanamsa` (default `"LAHIRI"`) for the division only. Pass `nakshatra_ayanamsa=None` to reproduce stored v5 values; it logs one warning per subject. See `references/zodiac-houses-perspectives.md`.
- **Lunar phase names moved onto the event.** `moon_phase_name` and `moon_emoji` now come from windows centred on the syzygies (New/Full ±6.4286°, quarters ±19.2857°) instead of 28 equal bins, so a moment near a boundary can report a different — closer — name than in v5. The `moon_phase` index (1–28) is unchanged, and `get_moon_phase_name_from_phase_int` / `get_moon_emoji_from_phase_int` remain the 28-bin approximation. See `references/utilities.md`.
- **`PlanetaryNodeModel.perihelion`/`aphelion` are deprecated** in favour of `periapsis`/`apoapsis` + `apsis_kind` — the old names say "the Sun", which is wrong for the Moon. They are still populated with the same objects, so reading code keeps working. No `DeprecationWarning` fires: it is a field-level deprecation, not a call site. See `references/mundane-events.md`.
