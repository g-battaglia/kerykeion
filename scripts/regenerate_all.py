...
def regenerate_charts() -> None:
    ...
    if not CHARTS_SCRIPT.exists():
        raise FileNotFoundError(f"Chart regeneration script not found at {CHARTS_SCRIPT}")
    ...
    if result.returncode == 0:
        ...
        print(result.stderr)
        raise RuntimeError("Chart regeneration failed")
    ...
    if MODERN_BASELINES_SCRIPT.exists():
        ...
        if result.returncode == 0:
            ...
            print(result.stderr)
            raise RuntimeError("Modern baseline regeneration failed")
    else:
